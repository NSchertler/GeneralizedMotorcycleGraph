...
bool MaxRectsBinPack::Insert(std::vector<RectSize> &rects, std::vector<Rect> &dst, FreeRectChoiceHeuristic method)
{
	dst.clear();
	dst.resize(rects.size());

	std::deque<size_t> openRectangles(rects.size());
	for (int i = 0; i < rects.size(); ++i)
		openRectangles[i] = i;
	while(!openRectangles.empty())
	{
		int bestScore1 = std::numeric_limits<int>::max();
		int bestScore2 = std::numeric_limits<int>::max();
		int bestRectIndex = -1;
		int bestOpenRectIndex = -1;
		Rect bestNode;

		for(size_t iOpenRect = 0; iOpenRect < openRectangles.size(); ++iOpenRect)
		{
			size_t i = openRectangles[iOpenRect];
			int score1;
			int score2;
		...
		openRectangles.erase(openRectangles.begin() + bestOpenRectIndex);
	}
	return true;
}
